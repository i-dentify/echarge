...
namespace ECharge.Services.Locations.Domain.Locations.Validation
{
    ...
    public class InviteUser : AbstractValidator<Locations.ViewModels.InviteUser>
    {
        #region [ Constructor ]

        public InviteUser(IQueryProcessor queryProcessor)
        {
            ...
            this.RuleFor(model => model.Email)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .NotEmpty()
                .WithMessage(LocationResources.RecipientEmailRequired)
                .MustAsync((model, email, cancellationToken) =>
                    queryProcessor.ProcessAsync(new InvitationForEmailDoesNotExist(model.Id, email), cancellationToken))
                .WithMessage(LocationResources.RecipientAlreadyInvited);
        }
